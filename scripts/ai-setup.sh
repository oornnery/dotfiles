...
# Link the five AI Stow packages. --migrate also archives the retired setup.
...
# Pi keeps runtime state (auth, sessions, packages) beside its config. Move any
# pre-existing real config aside so the repository copy can be linked in.
for name in settings.json AGENTS.md mcp.json agents prompts; do
    candidate="$target_dir/.pi/agent/$name"
    if [[ -e "$candidate" || -L "$candidate" ]]; then
        if [[ "$(readlink -f "$candidate" || true)" != "$repo_dir/pi/.pi/agent/$name" ]]; then
            archive "$candidate" "pi/$name"
        fi
    fi
done
...
for root in "$target_dir/.agents/skills" "$target_dir/.codex/agents" "$target_dir/.config/opencode" \
    "$target_dir/.claude" "$target_dir/.pi/agent"; do
    [[ -d "$root" ]] || continue
    ...
    done < <(find "$root" -type d \( -name node_modules -o -name npm \) -prune -o -type l -print0)
done
...
stow --no-folding -d "$repo_dir" -t "$target_dir" -R agents claude codex opencode pi
printf 'Applied shared skills, Claude Code, Codex, OpenCode and Pi to %s\n' "$target_dir"
