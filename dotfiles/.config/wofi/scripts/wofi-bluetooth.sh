...
# connect to bluetooth devices and display status infoo
#
...
print_status() {
    if power_on; then
        ...
        if (( $(echo "$(bluetoothctl version | cut -d ' ' -f 2) < 5.66" | bc -l) )); then
...
#wofi_command="wofi $* -p --style=$HOME/.config/wofi/style.widgets.css --conf=$HOME/.config/wofi/config.bluetooth"
wofi_command="wofi --dmenu $* --style=$HOME/.config/wofi/style.widgets.css --conf=$HOME/.config/wofi/config.bluetooth"
=
case "$1" in
    --status)
        print_status
